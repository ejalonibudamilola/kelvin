...
@Controller
@RequestMapping({"/allBusinessExecSummaryDetails.do"})
@SessionAttributes(types={WageBeanContainer.class})
public class ExecutivePayrollSummaryControllerDetails extends BaseController {
    ...
    @RequestMapping(method={RequestMethod.GET}, params={"fd", "td", "bc"})
    public String setupForm(@RequestParam("fd") String fromDate, @RequestParam("td") String toDate, @RequestParam("bc") BusinessCertificate bClient, Model model, HttpServletRequest request) throws Exception {
        ...
        return doWork(wGuideInfo, bc, model, request);
    }

    public String doWork(ArrayList<String> wGuideInfo, BusinessCertificate bc, Model model, HttpServletRequest request) throws IllegalAccessException, InstantiationException {
        ...
        for(String s : wGuideInfo){
            ...
            List<EmployeePayBean> wList = this.paycheckService.loadEmployeePayBeanByParentIdFromDateToDate(bc.getBusinessClientInstId(), wStartMonth, wStartYear, bc,null,false);
        ...
        List<SimulationMiniBean> wRetList = new ArrayList<>();
        wRetList = getSimulationMiniBeanFromMap(this.mdaMap,wRetList,true);

        List<SimulationMiniBean> cRetList = new ArrayList<>();
        List<SimulationInfoSummaryBean> cPayeeList = new ArrayList<>();
        ...
        //Now Set Deductions....
        wRetList = null;
        wRetList = this.getDetailsFromMap(this.payeeMap, new ArrayList<>(), true);

        List<SimulationInfoSummaryBean> wPayeeList = this.makeDeductionSimulationInfoSummaryBeanList(wRetList, new ArrayList<>(),this.PAYE);
        ...
        cRetList = this.getDetailsFromMap(this.contributoryMap, new ArrayList<>(), true);

        cPayeeList = this.makeDeductionSimulationInfoSummaryBeanListCont(cRetList, new ArrayList<>(),this.ContributoryPensions);

        cRetList = this.getDetailsFromMap(this.redemptionBondMap, new ArrayList<>(), true);
            ...
            List<SimulationMiniBean> pRetList, List<SimulationInfoSummaryBean> pPayeeList, Integer pTypeInd)
    {
        ...
        HashMap<Integer,SimulationInfoSummaryBean> wWorkMap = new HashMap<>();
        ...
        for(SimulationMiniBean child : pRetList){
            ...
            if(null == parent){
                ...
                List<SimulationMiniBean> wChildList = new ArrayList<>();
    ...
    {
        List<SimulationInfoSummaryBean> wRetList = new ArrayList<>();
    ...
    private void createSimulationInfoSummaryBean(
        ...
        for(EmployeePayBean e : pList){
            ...
            if(this.mdaMap.containsKey(wKey)){
                ...
                HashMap<Integer,SimulationMiniBean> wAll = new HashMap<>();
