...
@Controller
@RequestMapping({"/pensionContributoryReport.do"})
@SessionAttributes(types = {PaginatedBean.class})
public class PensionContributoryFormController extends BaseController {

    private final int pageLength = 20;
    private final String VIEW_NAME = "contributoryPensionForm";
...
//		List<EmpDeductMiniBean> paycheckDeductions = this.payrollServiceExt.loadPensionContributionByRunMonthAndYear((pageNumber - 1) * this.pageLength, this.pageLength, sortOrder, sortCriterion,wSDate.get(Calendar.MONTH), wSDate.get(Calendar.YEAR));
        ...
        try {
            ...
            paycheckDeductions = new ArrayList<EmpDeductMiniBean>();
        }
...
//	    double sumContributoryPension = this.payrollServiceExt.getTotalSumOfContPensionPerPayPeriod(wSDate.get(Calendar.MONTH), wSDate.get(Calendar.YEAR));
        double sumContributoryPension;
        try {
            sumContributoryPension = this.genericService.sumFieldUsingPredicateBuilder(IppmsUtils.getPaycheckClass(bc), predicateBuilder, Double.class, "contributoryPension", Arrays.asList("runMonth", "runYear"));
        } catch (NullPointerException e) {
            sumContributoryPension = 0.0D;
        }
        ...
        int wGLNoOfElements = this.genericService.countObjectsUsingPredicateBuilder(predicateBuilder1, EmployeePayBean.class);
