...
public class NewSimpleExcelGenerator extends BaseController implements Runnable {
    ...
    public NewSimpleExcelGenerator(ReportGeneratorBean rGB, HttpServletResponse httpServletResponse, HttpServletRequest httpServletRequest, int totalListSize) {
    ...
    public void run() {
        ...
        response.setHeader("Content-Disposition", "attachment; filename=" + reportGeneratorBean.getReportTitle() + ".xls");
...
//            addSheetTitle(workbook, sheet, reportGeneratorBean.getReportTitle(), reportGeneratorBean.getMainHeaders());
            ...
            int plusFive = reportGeneratorBean.getMainHeaders().size();
...
//            createHeaderRow(workbook, headerRow, reportGeneratorBean.getTableHeaders());
            ...
            if (reportGeneratorBean.getTableType() == 0) {
                ...
                ungroupedData(reportGeneratorBean.getTableHeaders(), reportGeneratorBean.getTableData(), workbook, sheet, reportGeneratorBean.getTotalInd(), plusFive);
            } else if (reportGeneratorBean.getTableType() == 1) {
//                firstSheetData(reportGeneratorBean.getTableHeaders(), reportGeneratorBean.getTableData(), workbook, sheet, reportGeneratorBean.getTotalInd(), plusFive);
                groupedData(reportGeneratorBean.getTableHeaders(), reportGeneratorBean.getTableData(), workbook, sheet, reportGeneratorBean.getGroupBy(), reportGeneratorBean.getMainHeaders(), path, reportGeneratorBean.getTotalInd(), plusFive, reportGeneratorBean.getGroupedKeySet());
    ...
    private void firstSheetData(List<Map<String, Object>> headers, List<Map<String, Object>> data, Workbook workbook, Sheet sheet, int totalInd, int plusFive) {
        ...
        for (Map<String, Object> datarow : data) {
            ...
            //calculate number of excel rows completed
        }
    ...
    public String getTitle(){
        return this.reportGeneratorBean.getReportTitle()+".xls";
    }
    ...
    public int getTotalRecords() {
        if(IppmsUtils.isNotNull(this.reportGeneratorBean.getGroupBy())) {
...
//            sumUngroupedRowCells(data, headers, workbook, sheet, rowNo);
    ...
    private int groupRows(List<Map<String, Object>> data, List<Map<String, Object>> headers, Workbook workbook,
        ...
        final int[] cellNo = {0};
        final int[] i = { 0 };
        final int[] thisRow = {rowNo};
        final Cell[] sn = new Cell[1];
        final Cell[] cell = {null};
        final CellStyle[] style = {workbook.createCellStyle()};
        final Double[] temp = new Double[1];
        int headerSize = headers.size();
        final String[] newTotal = new String[1];
        final int[] k = new int[rowNo];
...
//        }
        ...
        data.stream().forEach((m) -> {
            if (m.get(groupBy).toString().equals(tableValue)) {
                            thisRow[0] = thisRow[0] + 1;

                            row[0] = sheet.createRow(thisRow[0]);
                            sn[0] = row[0].createCell(0);
                            cellStyleNoBold(sn[0]);
                            sn[0].setCellValue(i[0] += 1);
                        } else {
                            row[0] = sheet.createRow(5);
                        }
                        headers.stream().forEach((h) -> {
                            if ((m.get(h.get("headerName").toString()) != null) && (m.get(groupBy).toString().equals(tableValue))) {

                                style[0].setWrapText(true);
                                cellNo[0] = cellNo[0] + 1;

                                cell[0] = row[0].createCell(cellNo[0]);
                                if (h.get("totalInd").toString().equals("2")) {
                                    temp[0] = Double.valueOf(PayrollHRUtils.removeCommas(m.get(h.get("headerName").toString()).toString()));
                                    newTotal[0] = PayrollHRUtils.getDecimalFormat().format(temp[0]);
                                    cell[0].setCellValue(IConstants.naira + newTotal[0]);
                                    cellStyleNoBold(cell[0]);
                                } else if (h.get("totalInd").toString().equals("3")) {

                                } else {
                                    cell[0].setCellValue(m.get(h.get("headerName").toString()).toString());
                                    cellStyleNoBold(cell[0]);
                                }
                            } else {
                                cellNo[0] = cellNo[0] + 1;
