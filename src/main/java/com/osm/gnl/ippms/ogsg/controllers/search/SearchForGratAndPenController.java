...
@Controller
@RequestMapping("/searchGratuityPensionCalc.do")
@SessionAttributes(types = HrMiniBean.class)
public class SearchForGratAndPenController extends BaseSearchController {
    ...
    public SearchForGratAndPenController() {
    }

        @RequestMapping(method = RequestMethod.GET)
        public String setupForm(Model model, HttpServletRequest
        request) throws HttpSessionRequiredException, EpmAuthenticationException {
        SessionManagerService.manageSession(request);
        HrMiniBean empHrBean = new HrMiniBean();

        empHrBean.setDisplayTitle("Search for Pensioner(s) Calculate Gratuity & Pension");
        ...
        public String processSubmit(@RequestParam(value = REQUEST_PARAM_CANCEL, required = false) String cancel, @ModelAttribute("empMiniBean") HrMiniBean pEHB, BindingResult
        ...
        if (emp.size() == 1) {
            ...
            Navigator.getInstance(IppmsEncoder.getSessionKey()).setFromForm("redirect:searchGratuityPensionCalc.do");
            return "redirect:calcPensionAndGratuity.do?eid=" + wHI.getParentId();
        } else {
            return "redirect:viewMultiEmployeeResults.do?eid="+ StringUtils.trimToEmpty(pEHB.getEmployeeId())+"&fn=" + IppmsUtils.treatNull(pEHB.getFirstName()) + "&ln=" + IppmsUtils.treatNull(pEHB.getLastName()) + "&noe=" + emp.size() + "&cn=17";
        }
