 ...
 * @Author Mustola
 * Since EPM days...
 * Modified for OGUN Consolidation
 * Sept 2020.
 */
...
public class PaycheckGenerator implements IPaycheckGenerator
{
  ...
  /**
   * This method is for ONLY Employees.
   ...
   * @throws Exception
   */
  public Object generatePaySlipModel(EmployeePayMiniBean empPayMiniBean, AbstractPaycheckEntity empPayBean, GenericService genericService, BusinessCertificate businessCertificate, Model pModel,
                                     ConfigurationBean configurationBean, PaySlipService pPaySlipService) throws Exception
  {
    ...
    this.paySlipService = pPaySlipService;
    ...
    //empPayMiniBean.setTaxesPaidYTD(empPayMiniBean.getTaxesPaidYTD() + empPayBean.getTaxesPaid());
    empPayMiniBean.setCurrentTaxesPaid(empPayBean.getTaxesPaid());
   // empPayMiniBean.setSalaryYTD(empPayBean.getTotalPay());
    ...
    if(empPayBean.getNetPay() == 0){
      ...
      empPayMiniBean.setBasicSalaryStr(empPayBean.getMonthlyBasicStr());
    ...
    if (empPayBean.getStatus().equalsIgnoreCase("P")) {
     ...
     empPayMiniBean.setPayPeriod( empPayBean.getPayPeriodStart().format(DateTimeFormatter.ofPattern("dd/MM/yyyy")) + " - " + empPayBean.getPayPeriodEnd().format(DateTimeFormatter.ofPattern("dd/MM/yyyy")));
    empPayMiniBean.setPayDate(empPayBean.getPayDate().format(DateTimeFormatter.ofPattern("dd/MM/yyyy")));
    ...
    List<NamedEntity> currDed = this.paySlipService.loadPaySlipDependentObjects(businessCertificate,empPayBean.getId(),IConstants.DEDUCTION);
    ...
    List<NamedEntity> currGarn =  this.paySlipService.loadPaySlipDependentObjects(businessCertificate,empPayBean.getId(),IConstants.LOAN);

    if ((currGarn != null) && (currGarn.size() > 0)) {
      EmpGarnMiniBean e ;
      for (NamedEntity p : currGarn) {
          e = new EmpGarnMiniBean();

        e.setName(p.getName());

        e.setCurrentGarnishment(p.getDeductionAmount());

        empPayMiniBean.setCurrentGarnTotal(empPayMiniBean.getCurrentGarnTotal() + p.getDeductionAmount());
        this.garnMap.put(e.getName(), e);
      }
    ...
    if (empPayBean.getContributoryPension() > 0.0D) {
      EmpDeductMiniBean egmb;
      if (this.pensionsMap.containsKey("Contributory Pension")) {
        egmb = this.pensionsMap.get("Contributory Pension");
      }
      else {
        egmb = new EmpDeductMiniBean();
        egmb.setName("Contributory Pension");
      }
      egmb.setCurrentDeduction(empPayBean.getContributoryPension());
      empPayMiniBean.setCurrentGarnTotal(empPayMiniBean.getCurrentGarnTotal() + empPayBean.getContributoryPension());
      this.pensionsMap.put( egmb.getName(), egmb);
    }



    if (empPayBean.getSalaryDifference() != 0.0D)
    {
      if (empPayBean.getSalaryDifference() > 0.0D) {
        EmpDeductMiniBean egmb;
        ...
        egmb.setCurrentDeduction(empPayBean.getSalaryDifference());
        //egmb.setYearToDate(empPayBean.getSalaryDifference());
        empPayMiniBean.setCurrentAllowanceTotal(empPayMiniBean.getCurrentAllowanceTotal() + empPayBean.getSalaryDifference());
        empPayMiniBean.setAllowanceTotal(empPayMiniBean.getAllowanceTotal() + empPayBean.getSalaryDifference());
        ...
        egmb.setCurrentGarnishment(empPayBean.getSalaryDifference());
        empPayMiniBean.setGarnishmentTotal(empPayMiniBean.getGarnishmentTotal() + empPayBean.getSalaryDifference() * -1.0D);
        empPayMiniBean.setCurrentGarnTotal(empPayMiniBean.getCurrentGarnTotal() + empPayBean.getSalaryDifference() * -1.0D);
        this.garnMap.put(egmb.getName(), egmb);
      }

    }

    if (empPayBean.getDevelopmentLevy() > 0.0D) {
      ...
      egmb.setCurrentGarnishment(egmb.getCurrentGarnishment() + empPayBean.getDevelopmentLevy());
      empPayMiniBean.setCurrentGarnTotal(empPayMiniBean.getCurrentGarnTotal() + empPayBean.getDevelopmentLevy());
      this.garnMap.put(egmb.getName(), egmb);
    }
    ...
    List<NamedEntity> wInstList = new ArrayList<>();
    ...
    if (empPayBean.isPayByDays()) {
      ...
      else if(empPayBean.getNoOfDays() > 1) {
        wInstList.add(new NamedEntity(businessCertificate.getStaffTypeName()+" was paid for " + empPayBean.getNoOfDays() + " days"));
      }else{
        wInstList.add(new NamedEntity(businessCertificate.getStaffTypeName()+" Not Paid. Terminated."));
      }
    }
		...
		if (empPayBean.getNetPay() == 0.0D) {
			if (empPayBean.getRejectedForPayrollingInd() == IConstants.ON) {
				...
				if (empPayBean.getSuspendedInd() == IConstants.ON)
					...
					if (empPayBean.isContractStaff()) {
						// Check if contract has expired...
						if (empPayBean.getHiringInfo().isContractExpired()) {
							wInstList.add(new NamedEntity(businessCertificate.getStaffTypeName()+" was not paid (On Contract)"));
							wInstList.add(new NamedEntity(
									"Contract Ended on " + empPayBean.getHiringInfo().getContractEndDateStr()));
						}
					}else if (empPayBean.isTerminationCandidate()) {
					   if(empPayBean.getNoOfDays() == 0){
                         wInstList.add(new NamedEntity(
                                 "Effective Termination Date - " + empPayBean.getTerminatedDate()));
                       }else{
					...
					}else if(empPayBean.isBirthDateTermination()) {
						wInstList.add(new NamedEntity(businessCertificate.getStaffTypeName()+" was not paid (Termination Candidate)"));
						wInstList.add(new NamedEntity(
                                businessCertificate.getStaffTypeName()+" Birth Date - " + empPayBean.getHiringInfo().getBirthDateStr()));
						wInstList.add(new NamedEntity(
								"Expected Retirement Date - " + empPayBean.getHiringInfo().getExpDateOfRetireStr()));
					}else if(empPayBean.isHireDateTermination()) {
						wInstList.add(new NamedEntity(businessCertificate.getStaffTypeName()+" was not paid (Termination Candidate)"));
						wInstList.add(new NamedEntity(
                                businessCertificate.getStaffTypeName()+" Hire Date - " + empPayBean.getHiringInfo().getHireDateStr()));
						wInstList.add(new NamedEntity(
								"Expected Retirement Date - " + empPayBean.getHiringInfo().getExpDateOfRetireStr()));
          ...
          if(empPayBean.getAllowanceRuleMaster() != null && !empPayBean.getAllowanceRuleMaster().isNewEntity()){
            wInstList.add(new NamedEntity(businessCertificate.getStaffTypeName()+" Had Pay Group Allowance Rule applied"));
            for(AllowanceRuleDetails details : empPayBean.getAllowanceRuleMaster().getAllowanceRuleDetailsList())
              wInstList.add(new NamedEntity(details.getBeanFieldName()+" - Actual = "+details.getMonthlyValueAsStr()+". Used = "+details.getApplyMonthlyValueStr()));
          }
        }
...
/*
    ...
    PayslipHelperBean payslipHelperBean = PaycheckGeneratorHelper.setPayGroupAllowances(empPayBean.getSalaryInfo(), empPayMiniBean, empPayBean, this.allowances,true);
    ...
    if(today.getYear() < empPayBean.getPayPeriodEnd().getYear()){
    	...
    	empYTDList = (List<AbstractPaycheckEntity>) genericService.loadAllObjectsUsingRestrictions(IppmsUtils.getPaycheckClass(businessCertificate),Arrays.asList(CustomPredicate.procurePredicate("employee.id", empId),
      ...
      for (AbstractPaycheckEntity e : empYTDList) {
        ...
        if (e.getTotalGarnishments() > 0.0D) {
         /* List<AbstractPaycheckGarnishmentEntity> pGarn = (List<AbstractPaycheckGarnishmentEntity>)genericService.loadAllObjectsUsingRestrictions(IppmsUtils.getPaycheckGarnishmentClass(businessCertificate),Arrays.asList(CustomPredicate.procurePredicate("employee.id", empId),
                  CustomPredicate.procurePredicate("payPeriodStart", yearStart, Operation.GREATER_OR_EQUAL),
                  CustomPredicate.procurePredicate("payPeriodEnd", empPayBean.getPayPeriodEnd(),Operation.LESS)),null);*/
          List<NamedEntity> pGarn = this.paySlipService.loadPaySlipDependentObjectsYTD(businessCertificate,empId,yearStart,today,IConstants.LOAN);
          for (NamedEntity p : pGarn) {
            EmpGarnMiniBean eGMB;
            if (!this.garnMap.isEmpty()) {
              eGMB = this.garnMap.get(p.getName());
              if (eGMB == null)
              {
                eGMB = new EmpGarnMiniBean();

                eGMB.setName(p.getName());
            ...
            eGMB.setYearToDate(eGMB.getYearToDate() + p.getDeductionAmount());
            empPayMiniBean.setGarnishmentTotal(empPayMiniBean.getGarnishmentTotal() + p.getDeductionAmount());
            this.garnMap.put(eGMB.getName(), eGMB);
        ...
        if (e.getContributoryPension() > 0.0D) {
          EmpDeductMiniBean egmb = null;
          if (this.pensionsMap.containsKey("Contributory Pension")) {
            egmb = this.pensionsMap.get("Contributory Pension");
          }
          ...
          egmb.setYearToDate(egmb.getYearToDate() + e.getContributoryPension());
          empPayMiniBean.setGarnishmentTotal(empPayMiniBean.getGarnishmentTotal() + e.getContributoryPension());
        ...
        if (e.getOtherArrears() > 0.0D) {
          EmpDeductMiniBean egmb;
          if (this.allowances.containsKey("Reabsorbing Arrears")) {
            egmb = this.allowances.get("Reabsorbing Arrears");
          }
          else {
            egmb = new EmpDeductMiniBean();
            egmb.setName("Reabsorbing Arrears");
          }
        ...
        payslipHelperBean = PaycheckGeneratorHelper.setPayGroupAllowances(e.getSalaryInfo(), empPayMiniBean, e,this.allowances, false);
        this.allowances = payslipHelperBean.getAllowances();
        empPayMiniBean = payslipHelperBean.getEmployeePayMiniBean();
      }
    } else {
    	 if(empPayBean.getNetPay() != 0)
           payslipHelperBean = PaycheckGeneratorHelper.setYTDAllowanceValues(empPayBean.getSalaryInfo(), empPayMiniBean, empPayBean, this.allowances);
      this.allowances = payslipHelperBean.getAllowances();
      empPayMiniBean = payslipHelperBean.getEmployeePayMiniBean();
    ...
    mandatoryList.addAll(allowList);

    Collections.sort(mandatoryList,Comparator.comparing(EmpDeductMiniBean::getName));
   ...
   // empPayMiniBean.setAllowanceList(allowList);
    ...
    if(empPayMiniBean.getCurrentNetPay() < 0.0D){
      empPayMiniBean.setNetPayColor("red");
    }else{
      empPayMiniBean.setNetPayColor("green");
    }
  ...
  //  pModel.addAttribute("allowanceList", allowList);
    ...
    pModel.addAttribute("employeeId", empId);
