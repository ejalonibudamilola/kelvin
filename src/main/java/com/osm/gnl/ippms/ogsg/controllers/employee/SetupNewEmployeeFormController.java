...
@Controller
@RequestMapping({"/setUpNewEmployee.do"})
@SessionAttributes(types = {EmployeeHrBean.class})
public class SetupNewEmployeeFormController extends BaseController {

    private final SetupNewEmployeeValidator validator;
    private final EmployeeService employeeService;
    private final PayrollService payrollService;
    private final String VIEW_NAME = "employee/setupNewEmployeeForm";


    @Autowired
    public SetupNewEmployeeFormController(SetupNewEmployeeValidator validator, EmployeeService employeeService, PayrollService payrollService) {
    ...
    public String setupForm(Model model, HttpServletRequest request) throws HttpSessionRequiredException, EpmAuthenticationException, InstantiationException, IllegalAccessException, UnsupportedEncodingException {
        SessionManagerService.manageSession(request, model);
        BusinessCertificate bc = getBusinessCertificate(request);

        EmployeeHrBean wEHB = new EmployeeHrBean();

        BiometricInfo biometricInfo = (BiometricInfo) request.getSession().getAttribute("biometricData");
        ...
        wEHB.setEmployeeId(biometricInfo.getEmployeeId());
        wEHB.setMiddleName(biometricInfo.getMiddleName());
        wEHB.setFirstName(biometricInfo.getFirstName());
        wEHB.setLastName(biometricInfo.getLastName());

        //Display Photo from API
        ...
        wEHB.setBiometricId(genericService.storeObject(biometricInfo));
    ...
    public String processSubmit(@RequestParam(value = "_cancel", required = false) String cancel,
        ...
        if (super.isButtonTypeClick(request, REQUEST_PARAM_CANCEL)) {
            ...
            BiometricInfo lastBio = new BiometricInfo(pEHB.getBiometricId());
            this.genericService.deleteObject(lastBio);
        ...
        wEmp.setMdaDeptMap(this.genericService.loadObjectUsingRestriction(MdaDeptMap.class, Arrays.asList(CustomPredicate.procurePredicate("mdaInfo.id", pEHB.getMdaId()),
       ...
       /* SetupEmployeeMaster wSEM = this.genericService.loadObjectUsingRestriction(SetupEmployeeMaster.class, Arrays.asList(getBusinessClientIdPredicate(request),
        ...
        if (!wSEM.isNewEntity()) {
            ...
            if (wErrorList != null && !wErrorList.isEmpty()) {
                super.addSessionAttribute(request, IConstants.EMP_SKEL, wEmp);
                super.addSessionAttribute(request, IConstants.EMP_SKEL_ERR, wErrorList);
                if(bc.isPensioner())
