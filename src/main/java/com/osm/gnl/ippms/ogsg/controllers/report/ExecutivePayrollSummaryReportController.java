...
import com.osm.gnl.ippms.ogsg.controllers.report.utility.ExecutivePayrollSummaryExcelGenerator;
import com.osm.gnl.ippms.ogsg.controllers.report.utility.ExecutivePayrollSummaryPdfGenerator;
...
@Controller
public class ExecutivePayrollSummaryReportController extends BaseController {
    ...
    @RequestMapping("/allBusinessExecSummaryReport.do")
    public void setupForm(@RequestParam("fd") String fromDate, @RequestParam("td") String toDate,
        ...
        doWork(wGuideInfo, bc, model, request, fromDate, toDate, response, reportType);
    }

    public void doWork(ArrayList<String> wGuideInfo, BusinessCertificate bc, Model model, HttpServletRequest request,
                         String fDate, String tDate, HttpServletResponse response, int reportType) throws IllegalAccessException, InstantiationException, IOException {

        SimulationInfoContainer wSIC = new SimulationInfoContainer();

        ExecutivePayrollSummaryExcelGenerator executivePayrollSummaryExcelGenerator = new ExecutivePayrollSummaryExcelGenerator();

        ExecutivePayrollSummaryPdfGenerator executivePayrollSummaryPdfGenerator = new ExecutivePayrollSummaryPdfGenerator();
        ...
        if(reportType == 1) {
            executivePayrollSummaryExcelGenerator.generateExcel(wSIC, "Executive Summary Report", response, request, bc);
        }
        else{
            executivePayrollSummaryPdfGenerator.generatePdf(wSIC, "Executive Summary Report", response, request, bc);
        }
