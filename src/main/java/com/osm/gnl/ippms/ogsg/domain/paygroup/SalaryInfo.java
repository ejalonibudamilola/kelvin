...
/**
 * @author ola Mustapha NOTE : Altering this mapping may affect the computation
 * of gratuity and pensions. check <code>
 * com.osm.gnl.pension.ogsg.hibernate.util.
 * PensionerGenerator.createSalaryInformation<code> (in the Pension
 * IPPMS)
 * DO NOT EDIT THIS CLASS EXCEPT BY EXPRESS APPROVAL FROM GNL Systems Ltd.
 */
@Entity
@Table(name = "ippms_salary_info")
@SequenceGenerator(name = "salaryInfoSeq", sequenceName = "ippms_salary_info_seq", allocationSize = 1)
@NoArgsConstructor
@Getter
@Setter
@Slf4j
public class SalaryInfo implements Serializable {
    ...
    @Id
    @GeneratedValue(generator = "salaryInfoSeq", strategy = GenerationType.SEQUENCE)
    @Column(name = "salary_info_inst_id", nullable = false, unique = true)
    private Long id;
    ...
    @SalaryAllowance(type = "Outfit Allowance", key = "N")
    ...
    @Transient private double basicMonthlySalary;
    ...
    @Transient private String swesAllowanceStr;
    @Transient private String swesAllowanceStrSansNaira;
    @Transient private String taxPercentageStr;
    @Transient private String toolsTorchLightStr;
    @Transient private String toolsTorchLightStrSansNaira;
    ...
    @Transient private double monthlyGrossSalary;
    ...
    public SalaryInfo(Long pId) {
        this.id = pId;
    }

    public SalaryInfo(Long pId, int pLevel, int pStep) {
        this.id = pId;
        this.level = pLevel;
        this.step = pStep;
    }
    ...
    public double getConsolidatedAllowance(){
        this.consolidatedAllowance = IppmsUtilsExt.getConsolidatedAllowance(this,false);
        return this.consolidatedAllowance;
    }
    public double getConsolidatedMonthlyAllowance(){
        this.consolidatedMonthlyAllowance = IppmsUtilsExt.getConsolidatedAllowance(this,true);
        return this.consolidatedMonthlyAllowance;
    }
    ...
    public String getBasicSalaryStr() {
        this.basicSalaryStr = naira + df
                .format(PayrollPayUtils.convertDoubleToEpmStandard(getMonthlyBasicSalary()));
        return basicSalaryStr;
    }

    public String getBasicSalaryStrSansNaira() {
        this.basicSalaryStrSansNaira = df
                .format(PayrollPayUtils.convertDoubleToEpmStandard(getMonthlyBasicSalary()));
        return basicSalaryStrSansNaira;
    }


    public String getMonthlyBasicSalaryStr() {
        this.monthlyBasicSalaryStr = (naira + df
                .format(PayrollPayUtils.convertDoubleToEpmStandard(getMonthlyBasicSalary() / 12.0D)));
        return this.monthlyBasicSalaryStr;
    }

    public Double getMonthlyBasicSalaryDouble() {
        this.monthlyBasicSalaryDouble = PayrollPayUtils.convertDoubleToEpmStandard(getMonthlyBasicSalary() / 12.0D);
    ...
    public String getMonthlyConsolidatedAllowanceStr() {
        this.monthlyConsolidatedAllowanceStr = naira + df
    ...
    public String getMonthlyGrossPayWivNairaStr() {
        this.monthlyGrossPayWivNairaStr = naira+df.format(getMonthlySalary());
    ...
    public String getMonthlySalaryStr() {
        if (this.monthlySalaryStr == null) {
            this.monthlySalaryStr = df.format(getMonthlySalary());
        }
    ...
    public String getSalaryScaleLevelAndStepStr() {
        this.salaryScaleLevelAndStepStr = (getSalaryType().getName() + " : " + getLevelStepStr());
    ...
    public String getToolsTorchLightStrSansNaira() {
        this.toolsTorchLightStrSansNaira = df.format(this.getToolsTorchLightAllowance());
        return this.toolsTorchLightStrSansNaira;
    }
    public String getToolsTorchLightStr() {
        this.toolsTorchLightStr = naira+df.format(this.getToolsTorchLightAllowance());
        return this.toolsTorchLightStr;
    }
...
//        if (this.nurseOtherAllowance > 0.0D)
    ...
    public double getBasicMonthlySalary() {
        if (this.getMonthlyBasicSalary() > 0) {
            this.basicMonthlySalary = PayrollPayUtils.convertDoubleToEpmStandard(this.basicMonthlySalary / 12.0D);
        }
        return basicMonthlySalary;
    }

    public boolean isPermanentSecretary() {
        permanentSecretary = this.salaryType.getConsolidatedInd() == IConstants.ON;
    ...
    public double getMonthlyGrossSalary() {
        monthlyGrossSalary = IppmsUtilsExt.getMonthlySalary(this);
