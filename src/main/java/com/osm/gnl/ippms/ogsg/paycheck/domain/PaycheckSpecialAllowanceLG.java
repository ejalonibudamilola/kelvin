...
@Entity
@Table(name = "ippms_paycheck_spec_allow_lg")
@SequenceGenerator(name = "paycheckSpecAllowLgSeq", sequenceName = "ippms_paycheck_spec_allow_lg_seq", allocationSize = 1)
@NoArgsConstructor
@Getter
@Setter
public class PaycheckSpecialAllowanceLG extends AbstractPaycheckSpecAllowEntity{

    @Id
    @GeneratedValue(generator = "paycheckSpecAllowLgSeq", strategy = GenerationType.SEQUENCE)
    ...
    public PaycheckSpecialAllowanceLG(Long pId) {
        this.id = pId;
    }
    ...
    public void setSpecialAllowanceInfo(AbstractSpecialAllowanceEntity abstractPaycheckSpecAllowEntity) {
        this.specialAllowanceInfo = (SpecialAllowanceInfoLG) abstractPaycheckSpecAllowEntity;
    }
}
